    ...
    const renderRowCell = (row: Row, cell: Cell, cellIndex: number) => {
        ...
        return (
            <TableCell
                ...
                classes={{
                    ...cellClasses,
                    bodyCell: `${cellClasses?.bodyCell ?? ''} BaseTable__Cell__${headCells[cellIndex].id}`,
                }}
                SortIcon={SortIcon}
            >
                        ...
                        <TableRow>
                            ...
                            {showIdCell && (
                                <TableCell
                                    ...
                                    align="left"
                                    sortDirection={sortByColumnId === '__id' ? sortDirection : 'desc'}
                            ...
                            {headCells.map((headCell) => (
                                <TableCell
                                    ...
                                    sortDirection={headCell.id === sortByColumnId ? sortDirection : 'desc'}
                                    SortIcon={SortIcon}
                                    onClick={() => {
                    ...
                    <TableBody data-testid="table-body">
                        {currentPageRows.map((row) => (
                            <TableRow
                                ...
                                className={selectedRowIdsState[row.id] ? 'BaseTable__Row--selected' : ''}
                            >
                                {selectable && (
                                ...
                                {showIdCell && (
                                    <TableCell classes={cellClasses} data-testid="table-row-cell" padding="none">
                                        {renderCellContent({ ...rowIdCell, label: row.id })}
                                    </TableCell>
                                )}
                                {row.cells
                                    .slice(0, headCells.length)
                                    .map((rowCell, cellIndex) => renderRowCell(row, rowCell, cellIndex))}
                            </TableRow>
                        ))}
                    </TableBody>
