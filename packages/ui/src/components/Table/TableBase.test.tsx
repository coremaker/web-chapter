...
describe('<Table />', () => {
    ...
    it('renders each cell of the row', () => {
        const { getByTestId } = render(<Table headCells={headCells} rows={rows.slice(0, 1)} />);
        const renderedRow = getByTestId(`table-body-row-${rows[0].id}`);

        rows[0].cells.forEach((cell) => expect(within(renderedRow).getByText(cell.label)).toBeInTheDocument());
    });

    it('renders column with default text "Id" and the row id', () => {
        const { getByText, getByTestId } = render(<Table showIdCell headCells={headCells} rows={rows.slice(0, 1)} />);
    ...
    it('renders an ellipsis icon in the last cell when rowActions are defined in props ', () => {
        ...
        const ellipsisButton = within(cells[cells.length - 1]).getByTestId(`ellipsis-button-${rows[0].id}`);
    ...
    it('renders the search input', () => {
        const { getByRole } = render(
            <Table makeSearchableRowContent={(row) => row.cells[0].label} headCells={headCells} rows={rows} />
        );
    ...
    it('filters the rendered rows based when the search input has any value', async () => {
        ...
        renderedRows.forEach((row) => {
            expect(within(row).getByText(rows[5].cells[0].label, { exact: false })).toBeInTheDocument();
        });
    });
    ...
    it('sorts alphabetically by any column when the column head is clicked', async () => {
        const { getByText, getAllByTestId } = render(
            <Table selectable showIdCell headCells={headCells} rows={rows} headIdCell={{ label: 'User ID' }} />
        );
